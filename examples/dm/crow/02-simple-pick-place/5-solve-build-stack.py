...
domain_f = 'simple-cube-in-box-domain.qddl'
problem_f = 'simple-cube-stack-problem.qddl'
...
def build_env(title='(Physical World)', is_local: bool = False):
    ...
    domain_qddl_filename = osp.join(osp.dirname(__file__), domain_f)
    problem_qddl_filename = osp.join(osp.dirname(__file__), problem_f)
...
def main(tcp_ports=DEFAULT_TCP_PORTS, ipc_ports=None):
    ...
    hat_domain = PyBulletSingleArmRWIPPSimplifiedDomain(robot, scene_metainfo, check_z_in_on_test=True, use_specialized_block_placement_sampler=True)
    ...
    goal = f'on(cubea, cubeb)'  # works
    goal = f'on(cubeb, cubec)'  # works
    goal = f'on(cubea, cubeb) and on(cubeb, cubec)' # fails
    goal = f'on(cubeb, cubec) and on(cubea, cubeb)'
