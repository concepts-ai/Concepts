...
from concepts.dsl.expression import ValueOutputExpression, FunctionApplicationExpression, AssignExpression, ConstantExpression, VariableExpression, ObjectConstantExpression, BoolExpression, QuantificationExpression
...
from concepts.pdsketch.operator import (
    RegressionRule, AchieveExpression, OperatorApplicationExpression, RegressionRuleApplier,
    gen_all_partially_grounded_actions, gen_all_grounded_regression_rules,
    SubgoalSerializability, SubgoalContSerializability
)
from concepts.pdsketch.planners.optimistic_search import instantiate_action

__all__ = [
    'enumerate_possible_symbolic_plans', 'enumerate_possible_symbolic_plans_regression_1', 'enumerate_possible_symbolic_plans_regression_c_v3',
    'surface_fol_downcast', 'ground_fol_expression', 'ground_operator_application_expression', 'evaluate_bool_scalar_expression',
    'TotallyOrderedPlan', 'PartiallyOrderedPlan',
    'gen_applicable_regression_rules_v3', 'extract_bounded_variables_from_nonzero', 'extract_bounded_variables_from_nonzero_dc'
]
...
def enumerate_possible_symbolic_plans_regression_1(
    ...
    for regression_rule in domain.regression_rules.values():
        ...
        if len(regression_rule.preconstraints) > 0:
            raise NotImplementedError('enumerate_possible_symbolic_plans_regression_1 does not support preconstraints.')
        if len(regression_rule.goal) > 1:
            raise NotImplementedError('enumerate_possible_symbolic_plans_regression_1 does not support multiple goal expressions.')
        goal = regression_rule.goal[0]
    ...
    def is_rule_applicable(state, regression_rule: RegressionRuleApplier, goal: FunctionApplicationExpression, csp: ConstraintSatisfactionProblem):
        goal_function = regression_rule.regression_rule.goal[0].assign_expr.predicate
...
def surface_fol_downcast(expression_1: ValueOutputExpression, expression_2: ValueOutputExpression) -> Optional[Dict[Variable, Union[Variable, ObjectConstant]]]:
    ...
    def dfs(expr1, expr2):
        ...
        if isinstance(expr1, VariableExpression):
            ...
            if isinstance(expr2, ObjectConstantExpression):
                ...
                return False
        elif isinstance(expr1, FunctionApplicationExpression):
            if not isinstance(expr2, FunctionApplicationExpression):
                return False
...
def ground_fol_expression(expression: ValueOutputExpression, variable_mapping: Dict[Variable, str]) -> ValueOutputExpression:
    ...
    name2symbol = {var.name: ObjectConstantExpression(ObjectConstant(name, var.dtype)) for var, name in variable_mapping.items()}
...
def ground_operator_application_expression(expression: OperatorApplicationExpression, variable_mapping: Dict[Variable, str], csp: Optional[ConstraintSatisfactionProblem] = None) -> OperatorApplicationExpression:
    ...
    for arg in expression.arguments:
        if isinstance(arg, VariableExpression):
            arguments.append(name2symbol[arg.variable.name])
        elif isinstance(arg, UnnamedPlaceholder):
            ...
            raise TypeError(f'Unknown argument type: {type(arg)}')
    return OperatorApplier(expression.operator, arguments)
...
class TotallyOrderedPlan(object):
    ...
    def __init__(self, sequence: Sequence[ValueOutputExpression], is_ordered: bool = True):
    ...
    def exclude(self, index: int):
        ...
        return TotallyOrderedPlan(self.sequence[:index] + self.sequence[index + 1:], self.is_ordered)
...
class PartiallyOrderedPlan(object):
    ...
    def __init__(self, chains: Sequence[TotallyOrderedPlan]):
        ...
        self.infeasible_chain_inedx = None
    ...
    def add_chain(self, chain: Sequence[ValueOutputExpression]) -> 'PartiallyOrderedPlan':
        """Add a new chain to the plan; return a new plan."""
        plan = PartiallyOrderedPlan(self.chains + (TotallyOrderedPlan(chain),))
        return plan

    @classmethod
    def from_single_goal(cls, goal: ValueOutputExpression):
        """Create a plan from a single goal."""
        return cls((TotallyOrderedPlan((goal,), True),))
...
def extract_bounded_variables_from_nonzero(
    ...
    tensor_variables = value.batch_variables
    exists_indices = [tensor_variables.index(v.name) for v in regression_rule.binding_arguments if v.quantifier_flag == 'exists']
    ...
    if not use_optimistic:
        ...
        if possibly_true_values.ndim == 0:
            ...
            for index, value in zip(indices, values.tolist()):
                exists_indices = tuple([index[i] for i in exists_indices])
                if value == 1:
