...
from concepts.pdsketch.operator import OperatorApplier, AchieveExpression, OperatorApplicationExpression
from concepts.pdsketch.planners.optimistic_search_bilevel_utils import(
    ground_fol_expression, ground_operator_application_expression, surface_fol_downcast,
    extract_bounded_variables_from_nonzero, extract_bounded_variables_from_nonzero_dc,
    TotallyOrderedPlan, PartiallyOrderedPlan
)

__all__ = [
    'gen_applicable_regression_rules', 'enumerate_possible_symbolic_plans_regression_c',
    'gen_applicable_regression_rules_v2', 'enumerate_possible_symbolic_plans_regression_c_v2'
]


def enumerate_possible_symbolic_plans_regression_c(
    ...
    def dfs(state, goal_set, maintains, csp, previous_actions, nr_high_level_actions=0):
        ...
        candidate_regression_rules = gen_applicable_regression_rules(executor, state, goal_set, maintains)
...
def gen_applicable_regression_rules(executor: PDSketchExecutor, state: State, goal_set: List[ValueOutputExpression], maintains: Set[ValueOutputExpression]):
