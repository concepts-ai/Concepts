...
import contextlib
from typing import Optional, Union, Sequence, Tuple, Set, Dict, Callable
...
from concepts.dsl.expression import BoolExpression, QuantificationExpression, BoolOpType
...
class Predicate(Function):
    ...
    def __init__(
        ...
        # self.is_derived has been coomputed in super().__init__
        ...
        # from concepts.pdsketch.strips.ao_discretization import AOFeatureCodebook
...
def flatten_expression(
    ...
    - The expression is a :class:`~concepts.dsl.expression.FunctionApplicationExpression`, and the sub-expression is a
        ...
        as arguments. The entire function application expression will be replaced by the Variable
...
class _FlattenExpressionVisitor(ExpressionVisitor):
    def __init__(
        self,
        ctx: ExpressionDefinitionContext,
    ...
    def visit_function_application_expression(self, expr: FunctionApplicationExpression) -> Union[VariableExpression, ValueOutputExpression]:
        ...
        self.ctx = ExpressionDefinitionContext(*expr.function.arguments)
        self.mappings = dict()
    ...
    def visit_quantification_expression(self, expr: QuantificationExpression) -> QuantificationExpression:
        with self.make_dummy_variable(expr.variable) as dummy_variable:
            return QuantificationExpression(expr.quantification_op, dummy_variable, self.visit(expr.expr))
    ...
    def visit_deictic_assign_expression(self, expr: DeicticAssignExpression) -> DeicticAssignExpression:
        with self.make_dummy_variable(expr.variable) as dummy_variable:
            return type(expr)(dummy_variable, self.visit(expr.expr))
