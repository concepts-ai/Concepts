...
from concepts.dsl.constraint import Constraint, ConstraintSatisfactionProblem, OptimisticValue, Assignment, AssignmentType, AssignmentDict, SimulationFluentConstraintFunction
...
from concepts.dsl.expression import is_and_expr, iter_exprs
...
__all__ = ['CSPNotSolvable', 'CSPNoGenerator', 'csp_dpll_sampling_solve', 'csp_dpll_simplify']

ConstraintList = List[Optional[Constraint]]
...
def _apply_assignments(executor: PDSketchExecutor, constraints: ConstraintList, assignments: Dict[int, Assignment]) -> ConstraintList:
    ...
    for c in constraints:
        ...
        nc = Constraint(c.function, new_args, new_rv, note=c.note)
...
def _check_constraint(executor: PDSketchExecutor, c: Constraint) -> bool:
    ...
    if c.function is BoolOpType.NOT:
        ...
        if c.function.is_generator_placeholder:
            return False
        func = executor.get_function_implementation(f'predicate::{c.function.name}')
...
def _filter_deterministic_equal(executor: PDSketchExecutor, constraints: ConstraintList, assignments: Dict[int, Assignment]) -> Tuple[bool, ConstraintList]:
    ...
    for i, c in enumerate(constraints):
        if c.is_equal_constraint and isinstance(c.rv, TensorValue):
            ...
            if c.rv.item():
                ...
                if c.arguments[0].dtype == BOOL:
                    constraints[i] = Constraint(BoolOpType.NOT, [c.arguments[0]], c.arguments[1], note=c.note)
                    progress = True
    if progress:
        return progress, _apply_assignments(executor, constraints, assignments)
    return progress, constraints


def _filter_unused_rhs(executor: PDSketchExecutor, constraints: ConstraintList, assignments: Dict[int, Assignment], index2actionable: Dict[int, bool]) -> ConstraintList:
    ...
    for i, actionable in index2actionable.items():
        if actionable:
            used[i] += 100
    ...
    return _apply_assignments(executor, constraints, assignments)


def _filter_deterministic_clauses(executor: PDSketchExecutor, constraints: ConstraintList, assignments: Dict[int, Assignment]) -> Tuple[bool, ConstraintList]:
...
def _find_bool_variable(executor: PDSketchExecutor, constraints: ConstraintList, assignments: Dict[int, Assignment]) -> Optional[int]:
...
def _find_grounded_function_application(executor: PDSketchExecutor, constraints: ConstraintList) -> Optional[Constraint]:
...
def _find_typegen_variable(executor: PDSketchExecutor, dtype: ValueType) -> Optional[Generator]:
    assert isinstance(dtype, NamedTensorValueType)
    for g in executor.domain.generators.values():
...
def _find_fancy_gen_variable(executor: PDSketchExecutor, csp: ConstraintSatisfactionProblem, constraints: ConstraintList, assignments: AssignmentDict) -> Optional[List[Tuple[List[Constraint], Dict[int, Union[TensorValueTypeBase, PyObjValueType]], FancyGenerator]]]:
    results = list()
...
def _find_gen_variable_combined(executor: PDSketchExecutor, csp: ConstraintSatisfactionProblem, constraints: ConstraintList, assignments: AssignmentDict) -> GeneratorMatchingReturnType:
    ...
    for name, dtype in csp.index2type.items():
        if name not in assignments and isinstance(dtype, NamedTensorValueType):
            g = _find_typegen_variable(executor, dtype)
            if g is not None:
...
def csp_dpll_sampling_solve(
    ...
    def dfs(constraints, assignments):
        ...
        progress = True
        while progress:
            progress, constraints = _filter_deterministic_equal(executor, constraints, assignments)
        if enable_ignore:
            constraints = _filter_unused_rhs(executor, constraints, assignments, csp.index2actionable)
        progress = True
        while progress:
            progress, constraints = _filter_deterministic_clauses(executor, constraints, assignments)
        ...
        if (next_bool_var := _find_bool_variable(executor, constraints, assignments)) is not None:
            assignments_true = assignments.copy()
            assignments_true[next_bool_var] = Assignment(AssignmentType.VALUE, True)
            try:
                constraints_true = _apply_assignments(executor, constraints, assignments_true)
            ...
            try:
                constraints_false = _apply_assignments(executor, constraints, assignments_false)
                return dfs(constraints_false, assignments_false)
            except CSPNotSolvable:
                pass

            raise CSPNotSolvable()
        elif (next_fapp := _find_grounded_function_application(executor, constraints)) is not None:
            function: Predicate = next_fapp.function
            arguments = next_fapp.arguments

            external_function = executor.get_function_implementation(f'predicate::{function.name}')
            ...
            try:
                ...
                new_constraints = _apply_assignments(executor, new_constraints, new_assignments)
            ...
            raise CSPNotSolvable()
        elif (next_gen_vars := _find_gen_variable_combined(executor, csp, constraints, assignments)) is not None:
