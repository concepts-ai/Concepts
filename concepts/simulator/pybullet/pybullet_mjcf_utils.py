...
class MJCFcanonicalizer(object):
    ...
    def _canonicalize_include_inner(self, object):
        found = False
        for k in list(object.keys()):
            if k == 'include':
                ...
                if isinstance(v, list):
                    for sub_v in v:
                        self._canonicalize_include_append(object, sub_v)
                else:
                    self._canonicalize_include_append(object, v)
            elif not k.startswith('__'):
                v = object[k]
                if isinstance(v, list):
                    for sub_v in v:
                        found = found or self._canonicalize_include_inner(sub_v)
                else:
                    found = found or self._canonicalize_include_inner(v)
        return found

    def _canonicalize_include_append(self, object, include_obj):
    ...
    def _canonicalize_include(self, object):
        for i in range(100):
            found = self._canonicalize_include_inner(object)
            if not found:
                break

    def _canonicalize_directories(self, object):
    ...
    def _canonicalize_attributes(self, object):
        for k, v in object.items():
            if not k.startswith('__'):
                if isinstance(v, list):
                    for sub_v in v:
                        self._canonicalize_attributes(sub_v)
                else:
                    self._canonicalize_attributes(v)
    ...
    def canonicalize(self):
        self._canonicalize_include(self.xml_object)
        self._canonicalize_directories(self.xml_object)
        self._canonicalize_attributes(self.xml_object)
        return self.xml_object


def canonicalize_mjcf(filename, xml_object=None):
    return MJCFcanonicalizer(filename, xml_object=xml_object).canonicalize()
