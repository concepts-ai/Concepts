...
from concepts.utils.interpolation_utils import gen_cubic_spline, get_next_target_cubic_spline
from concepts.utils.interpolation_utils import gen_linear_spline, get_next_target_linear_spline

from concepts.simulator.pybullet.client import BulletClient
from concepts.simulator.pybullet.world import BodyFullStateSaver, ConstraintInfo
from concepts.simulator.pybullet.components.robot_base import Robot, GripperObjectIndices, RobotActionPrimitive
from concepts.simulator.pybullet.rotation_utils import rotate_vector, quat_mul, quat_conjugate
...
class PandaRobot(Robot):
    ...
    def get_robot_ee_link_id(self) -> int:
    ...
    def _init_ikfast(self):
        if self.ik_fast_wrapper is None:
            ...
            import concepts.simulator.pybullet.ikfast.franka_panda.ikfast_panda_arm as ikfast_module
    ...
    def rrt_collision_free(self, pos1: np.ndarray, pos2: Optional[np.ndarray] = None, smooth_fine_path: bool = False, disable_renderer: bool = True, **kwargs):
        ...
        with self.world.save_world_v2(), jacinle.cond_with(self.client.disable_rendering(suppress_stdout=False), disable_renderer):
            path = birrt(cfree_pspace, pos1, pos2, smooth_fine_path=smooth_fine_path, **kwargs)
            if path[0] is not None:
                return True, path[0]
            return False, None

    def move_qpos(self, target_qpos, speed=0.01, timeout=10, gains=1, local_smoothing: bool = True) -> bool:
        last_qpos = None
