...
def export_link(link: LinkBase) -> ET.Element:
    """
...
def export_joint(joint: JointBase) -> list[ET.Element]:
    """
    ...
    """
    if joint.get_parent_link() is None:  # joint from root link to first robot link
        type = "fixed" if (joint.type == "fixed" or joint.type == 'unknown') else "floating"
        elem_joint = ET.Element("joint", {"name": "__root_joint__", "type": type})
        ET.SubElement(elem_joint, "parent", {"link": "__world__"})
        ET.SubElement(elem_joint, "child", {"link": joint.get_child_link().name})
        return [elem_joint]
    ...
    if joint.type == "fixed":
        ...
        if joint.get_limits()[0][0] < -1e5 and joint.get_limits()[0][1] > 1e5:
            type = "continuous"
        else:
            type = "revolute"
    else:
    ...
    j2p = joint.get_pose_in_parent()
    c2j = joint.get_pose_in_child().inv()

    # dummy link at joint frame
    elem_dummy_link = ET.Element(
        "link", {"name": f"link_dummy_{joint.get_child_link().get_index()}"}
    )
    ...
    ET.SubElement(elem_joint, "parent", {"link": joint.get_parent_link().name})
    ET.SubElement(elem_joint, "child", {"link": f"link_dummy_{joint.get_child_link().get_index()}"})
    if type == "prismatic" or type == "revolute" or type == "continuous":
        limit_attrib = {
            "effort": str(min(joint.force_limit, 1e5)),
            "velocity": "0",
            "lower": str(max(joint.get_limits()[0][0], -1e5)),
            "upper": str(min(joint.get_limits()[0][1], 1e5)),
        }
        if type == "continuous":
    ...
    elem_dummy_joint = ET.Element(
        "joint", {"name": f"joint_dummy_{joint.get_child_link().get_index()}", "type": "fixed"}
    )
    ...
    ET.SubElement(
        elem_dummy_joint, "parent", {"link": f"link_dummy_{joint.get_child_link().get_index()}"}
    )
    ET.SubElement(elem_dummy_joint, "child", {"link": joint.get_child_link().name})

    return [elem_joint, elem_dummy_link, elem_dummy_joint]


def export_kinematic_chain_xml(articulation: ArticulationBase) -> ET.Element:
...
def export_kinematic_chain_urdf(
    articulation: ArticulationBase, force_fix_root: bool = False
) -> str:
    """
