...
from concepts.math.rotationlib_xyzw import enumerate_quaternion_from_vectors, quat_mul, rotate_vector, wxyz2xyzw, xyzw2wxyz, \
    cross, find_orthogonal_vector, quaternion_from_axes as get_quaternion_from_axes

from magic.tasks.mug_scene import MugScene
from magic.tasks.table_scene import TableScene
from magic.sapien_utils import get_contacts_by_id, get_contacts_with_articulation  # , ActorSaver
...
def gen_qpos_to_qpos_trajectory(
    ...
    if idx is None:
        planner = table_scene.planner
        assert planner is not None, 'the default planner is None, please identify the planner to use'
    else:
        planner = table_scene.planner_1 if idx == 1 else table_scene.planner_2
        assert planner is not None, 'the indicated planner is None'
    if ignore_env:
        table_scene.planner.remove_point_cloud()
    else:
        table_scene.update_env_pcd(exclude_ids=exclude_ids, pcd_resolution=pcd_resolution, verbose=verbose, idx=idx)
...
def gen_qpos_to_pose_trajectory(
    ...
    end_pose_rel_robot_base = None
    if idx is None:
        planner = table_scene.planner
        assert planner is not None, 'the default planner is None, please identify the planner to use'
        end_pose_rel_robot_base = end_pose
    else:
        planner = table_scene.planner_1 if idx == 1 else table_scene.planner_2
        robot: Articulation = table_scene.robot_1 if idx == 1 else table_scene.robot_2
        assert planner is not None, 'the indicated planner is None'
        end_pose_rel_robot_base = robot.get_root_pose().inv() * Pose(end_pose.p, end_pose.q)
        end_pose_rel_robot_base = pymp.Pose(end_pose_rel_robot_base.p, end_pose_rel_robot_base.q)
    if ignore_env:
        planner.remove_point_cloud()
    else:
        table_scene.update_env_pcd(exclude_ids=exclude_ids, pcd_resolution=pcd_resolution, idx=idx, verbose=False)
    if not use_screw:
        result = planner.plan_pose(
            end_pose_rel_robot_base,
            start_qpos,
