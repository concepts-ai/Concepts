...
from concepts.vision.george_vision_pipeline.segmentation_models import random_colored_mask, visualize_instance_segmentation, ImageBasedPCDSegmentationModel, PointGuidedImageSegmentationModel, InstanceSegmentationResult
from concepts.math.rotationlib_wxyz import axisangle2quat, quat2mat
...
class CameraTransformation(object):
    ...
    def c2w(self, u: Union[float, np.ndarray], v: Union[float, np.ndarray], d: Union[float, np.ndarray]):
        ...
        if isinstance(u, float) and isinstance(v, float) and isinstance(d, float):
            ...
            x, y, z, d = np.dot(np.linalg.inv(self.extrinsics), np.array([x, y, z, 1]))
            return x / d, y / d, z / d
        else:
...
class ObjectCentricVisionPipeline(object):
    ...
    def manual_set_pcd(self, pcd: o3d.geometry.PointCloud):
        self._pcd = pcd
    ...
    def construct_object_segmentation(
        ...
        for obj in detected_objects:
            ...
            if np.asarray(obj.pcd.points).shape[0] < 10:
                if verbose:
                    print('Object', obj.label, 'has too few points, skipping.')
                continue
...
def get_pointcloud_multicam(color_images: np.ndarray, depth_images: np.ndarray, camera_transformations: List[CameraTransformation]) -> o3d.geometry.PointCloud:
    """Reconstruct a colored point cloud from a color and depth image.

    Args:
        color_images: a numpy array of shape (N, H, W, 3) representing the color image.
        depth_images: a numpy array of shape (N, H, W) representing the depth image.
        camera_transformations: N CameraTransformation object.
    ...
    all_points = list()
    all_colors = list()
    for color_image, depth_image, camera_transformation in zip(color_images, depth_images, camera_transformations):
        xs = np.arange(0, color_image.shape[1])
        ys = np.arange(0, color_image.shape[0])
        xx, yy = np.meshgrid(xs, ys)
        xx = xx.flatten()
        ...
        xx, yy, zz = camera_transformation.c2w(xx, yy, zz)
        all_points.append(np.stack([xx, yy, zz], axis=1))
